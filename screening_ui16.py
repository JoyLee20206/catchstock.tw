...
> 📌 **為什麼門檻不是固定的?** 2018~2020 區間約 ±15k,2024 後外資總部位放大,常見 ±20k 以上,
> 所以**門檻會隨市值規模演進**(目前以 2024~2025 校準,未來可能再調)。
>
> 外資期貨被視為「聰明錢的方向盤」,持續強空往往伴隨現貨賣壓。但要避免**單日翻轉就追**,看趨勢比看單日重要。

---

### 7️⃣ 👥 散戶估算(微台反向推導,小台備援)
...
> 微台合約值約 1/10 小台,基本上**只有散戶會玩**(法人覺得太小),
> 純度高達 ~ 90%,訊號雜訊最低。
>
> ⚠️ 微台合約小、口數天然較大(常見 ±2~3 萬),所以門檻也比小台時代寬。
> 萬一某日只抓得到小台,雜訊較高但方向仍可參考。
>
