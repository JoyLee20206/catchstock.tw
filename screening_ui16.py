            ...
            with _sub_validate:
                ...
                if _ex.get("error"):
                    ...
                    if _stop5 and _fixed:
                        ...
                        st.success(
                            f"💡 **停損是「控制風險」,不是提高報酬**:最慘單筆從 {_fixed['worst']:.0f}% "
                            f"縮到 {_stop5['worst']:.0f}%(少賠 {abs(_worst_cut):.0f} 個百分點,這保護最穩定),"
                            f"代價是勝率降到 {_stop5['win_rate']*100:.0f}%。"
                            f"  \n⚠️ 名次會隨樣本變,別追第一名——停損以「防大跌」為目的。"
                            f"選股表/推播已標「🛑停損價(現價−5%)」。"
                        )
                    if _ex["n"] < 30:
                        st.caption(
