...
### 🌡️ 第五區：大盤情緒指標(溫度計)
* **這是什麼?** 整合 **7 個市場訊號** 算出一個 0~100 的「市場溫度」,幫你判斷現在是「該積極選股」還是「該保守觀望」。
    ...
    if _s and _s.get("temperature") is not None:
        ...
        _CARD_DEFS = [
            ...
            ("fi_futures",     "🏦 外資期貨",        lambda v: f"{v['value']:+,}口" if v.get('value') is not None else "N/A",
             ...
             "外資大台期貨未平倉淨口數(TAIFEX)。±5k=中性、±20k=顯著(2024~2025 規模校準)。"),
            ("retail_futures", "👥 散戶估算",
             lambda v: (f"{int(v['pct_rank'])}%位"
                        if v.get('pct_rank') is not None
                        else (f"{v['value']:+,}口" if v.get('value') is not None else "N/A")),
             lambda v: (f"{v['label']}"
                        + (f" · {v['value']:+,}口" if v.get('pct_rank') is not None and v.get('value') is not None else "")
                        + (f" · 累積{v['n_days']}日" if v.get('n_days', 0) > 0 and v.get('pct_rank') is None else "")),
             "微台優先(散戶占比~90%最純),小台備援。三大法人淨口數取反估散戶方向。"
             "累積 ≥ 20 日後改用 90 日歷史百分位(更穩健),不足時暫用絕對門檻。"),
        ]
...
| 指標 | 權重 | 為什麼這樣設 |
|---|---|---|
| 🇺🇸 美股 VIX | 20% | 全球恐慌錨,台股開盤先看它 |
| 🇹🇼 台指波動率 | 10% | 在地恐慌訊號 |
| 📐 大盤位階 | 20% | 過熱/超跌的關鍵 |
| 📉 融資週變化 | 10% | 散戶當週方向(反指標) |
| 💰 融資水位 | 10% | 散戶槓桿水位(反指標) |
| 🏦 外資期貨 | 20% | 主力期貨佈局,聰明錢方向盤 |
| 👥 散戶估算 | 10% | 散戶期貨方向(反指標) |
...
- **算法**:`(近 5 日總融資餘額平均 - 前 5 日平均) / 前 5 日平均 × 100`
- **判讀**(反指標):
...
> 📌 「融資」 = 借錢買股票的槓桿部位,散戶占大宗。
> **散戶急著加碼往往在頭部、急著認賠往往在底部**,所以這是反指標。

---

### 5️⃣ 💰 融資水位

- **資料來源**:同 margin parquet
...
> 📌 跟「週變化」的差別:**水位看「絕對位階」、週變化看「最近方向」**。
> 兩者搭配:水位高 + 週又急增 = 雙重過熱警告。

---

### 6️⃣ 🏦 外資期貨(大台 TX 未平倉淨口數)
...
**模式 A — 百分位制(累積 ≥ 20 日後啟用,主要評分方式)**

把當前散戶淨口數放進「過去 90 日的散戶淨口數分布」算百分位,反指標:

| 百分位 | 標籤 | 分數 | 解讀 |
|---|---|---|---|
| > 80% | 散戶極多 🔴 | 25 | 比近 3 個月任何時候都多單 → **反向警訊** |
| 60~80% | 散戶偏多 🟠 | 40 | |
| 40~60% | 中性 🟡 | 55 | |
| 20~40% | 散戶偏空 🟢 | 65 | |
| < 20% | 散戶極空 🟢 | 75 | **反向利多訊號** |

**模式 B — 絕對門檻(歷史不足 20 日的過渡用,2025~2026 微台校準)**
...
|---|---|---|
| > +30,000 | 散戶多 🟠 | 40 |
| +10,000~+30,000 | 略偏多 🟡 | 48 |
| -10,000~+10,000 | 中性 🟢 | 55 |
| -30,000~-10,000 | 略偏空 🟢 | 62 |
| < -30,000 | 散戶空 🟢 | 70 |

卡片上會顯示「累積 X 日」提示累積進度,**累積到 20 日後自動切換到百分位制**。
