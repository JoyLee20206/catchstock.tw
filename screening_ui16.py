...
> 📌 大盤情緒、資料更新等說明,請看**首頁上方對應的分頁**(🌡️ 大盤情緒、🔄 資料更新)。
    ...
    st.slider(labeled("過關門檻 PASS_SCORE", 'pass_score'), 1, 11, key='pass_score',
              help="滿分 11 分，達標才算入榜。\n\n計分包含：法人買超、大戶增散戶減（各 2 分，訊號歸因唯二正 edge）、技術突破、KD金叉、營收成長與大盤相對強弱。券相關 2026-06 起停用觀察（只記錄不計分）。\n\n💡 現行門檻 8（2026-06-13 由 7 調高，7 分一天選出 83 檔太寬）：8 分=單籌碼+KD/營收其一，9 分=大戶+散戶雙籌碼；沒有籌碼訊號的股票進不來。大盤破季線會再自動 +1（空頭從嚴）。")
        ...
        if not overall:
            ...
            with _sub_validate:
                ...
                if by_score:
                    ...
                    st.caption(
                        "🔥 9 分以上 = 頂級(滿分 11 制:大戶↑/散戶↓各 2 分、券停用;9 分必含雙籌碼共振)"
                        "｜ ✅ 8 分 = 合格(現行門檻,2026-06-13 起)｜ ⚠️ 7 分以下 = 舊門檻/降標紀錄。"
                        "　註:改制(2026-06)前的舊紀錄為滿分 10 制、門檻 7,跨期比較請留意。"
                        "　每次入選算一筆(同檔不同天分開計),故筆數比明細表的檔數多是正常的。"
                    )
                    # 分數 → 標籤對映 (滿分 11 制、門檻 8;舊 10 制/門檻 7 的紀錄跨期僅供參考)
