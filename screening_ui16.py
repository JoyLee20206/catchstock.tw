...
with _tab_bt:
    st.caption(
        "對 daily 快取掃描 10 種訊號的歷史觸發點,算「進場後 N 日報酬」── "
...
| 訊號 | 觸發條件 | 適用場景 |
|---|---|---|
| **量價齊揚突破** | close ≥ 60 日新高 × 0.995 且 量 ≥ MA20 × 1.5 | 動能突破策略 |
| **外資連 5 日買超** | 近 5 日外資總淨額 > 0 且 ≥ 2 日買 | 跟著大資金 |
| **KD 低檔金叉** | K 上穿 D 且 K < 設定門檻(低檔) | 抓跌深反彈 |
| **投信連 5 日買超** | 近 5 日投信總淨額 > 0 且 ≥ 2 日買 | 中小型股偏好 |
| **籌碼共振(散戶↓)** | 本週散戶比例 < 上週 | 主力吃貨訊號 |
| **三大法人同步買超** | 同日 外資+投信+自營 全買 | 最強共識訊號 |
| **MA20 拉回守穩** | 多頭中拉回觸碰 MA20 後紅 K | 葛蘭碧第二法則 |
| **月營收雙紅突破** | YoY > 10% 且 MoM > 0 且 當日突破新高 | **基本面 + 技術面** |
| **MA 黃金交叉** | MA20 上穿 MA60(中長線轉折) | 稀少但可靠 |
| **品質突破** | 突破 + 突破前 10 日有 ≥ 6 日量縮 | 整理蓄勢後爆量 |
...
**1. 只勾「量價齊揚突破」**
- 在問:「只看股價突破新高 + 帶量,這招到底能不能賺?」
- 適合:想驗證最經典的技術派策略有沒有效

**2. 勾「KD 低檔金叉」+「MA20 拉回守穩」→ 選 AND**
- 在問:「股價跌深後 KD 翻揚,又站回月線,這時候進場會賺嗎?」
- 適合:想抓「跌深反彈、波段起漲第一根」的人

**3. 勾「外資連 5 日買超」+「投信連 5 日買超」→ 選 AND**
- 在問:「外資跟投信兩家法人都連續買的股票,跟著進場能不能賺?」
- 適合:跟單派(讓大資金當你的領頭羊)

**4. 只勾「三大法人同步買超」**
- 在問:「外資、投信、自營商三家『同一天』都買超的股票,後續表現怎樣?」
- 適合:只想看最強共識訊號(但樣本會比較少)

**5. 勾「籌碼共振(散戶↓)」+「量價齊揚突破」→ 選 AND**
- 在問:「主力先默默吃貨(散戶被甩開),然後股價才突破新高,這種能不能賺?」
- 適合:相信「籌碼面比技術面早一步」的人

**6. ⭐ 只勾「月營收雙紅突破」(本系統最強的單一訊號之一)**
- 在問:「公司營收年增 + 月增,同一天股價又突破新高,跟著進場勝率有多高?」
- 適合:基本面派,只想找「真材實料 + 技術面同步啟動」的股票

**7. ⭐ 勾「月營收雙紅突破」+「外資連 5 日買超」→ 選 AND**
- 在問:「業績好的股票 + 外資也在掃貨,這種雙重確認的勝率多高?」
- 適合:**保守穩健派的最佳組合**,基本面+籌碼面雙重把關

**8. 勾「品質突破」單獨**
- 在問:「整理量縮一陣子後才爆量突破的股票,跟『一路飆然後勉強突破』比起來真的比較會漲嗎?」
- 適合:想驗證「不是所有突破都一樣」的概念

**9. 只勾「MA 黃金交叉」**
- 在問:「MA20 上穿 MA60 的中長線轉折日進場,後續報酬如何?」
- 適合:中長線投資人(訊號稀少,但每個都是大轉折點)

**10. 全部 10 個訊號都勾起來 → 選 OR(寬鬆模式)**
- 在問:「不管哪個訊號觸發都當作買進機會,整體勝率長怎樣?」
- 適合:當作「基準對照組」,看你篩選後的策略到底有沒有比這個強
...
打開下面那個摺疊區,**同一組訊號**會跑 5/10/20/40 日對比 → 直接看到「**這策略最佳持有期是幾天?**」
    ...
    if _bt.get("error"):
        ...
        # ── 10 訊號對照圖 ──
        st.divider()
        st.markdown(f"**10 訊號 {hold_choice} 日勝率 / 平均報酬對照**")
