...
| 組合 | 意義 | 模式 |
|---|---|---|
| `breakout` 單獨 | 純技術突破能不能賺? | — |
| `kd_cross + ma20_pullback` AND | 低檔啟動 + 守穩 MA20 | AND 嚴格 |
| `foreign + investment_trust` AND | 外資投信同時買超 | AND 嚴格 |
| `triple_buy` 單獨 | 最強共識(樣本少) | — |
| `resonance + breakout` AND | 籌碼集中後突破 | AND 嚴格 |
| 你目前的策略全勾 OR | 看看「任一訊號」綜合表現 | OR 寬鬆 |
