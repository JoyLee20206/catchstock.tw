...
#   4. 券相關合併(0,停用觀察)  5. 400張大戶上升(2)  6. 散戶下降(2)
#   7. 技術面三合一(1) 8. KD 低檔金叉(1)     9. 月營收達標(1)
#   10. RS vs 大盤(1) (僅多頭計分;空頭時記 0 — 見動態門檻調整)
#   ※ 大戶/散戶加重為 2 分:訊號歸因唯二明顯正 edge(+9.2%/+6.4%)。
#     門檻 8(2026-06-13 由 7 調高;7 分=基本盤+單籌碼,實測一天 83 檔太寬):
#     8 分=單籌碼+KD/營收其一、9 分=雙籌碼共振;無籌碼訊號(最高 5+1+1=7)進不來
#   ※ 券相關 2026-06-12 起停用計分(歸因 edge -6.34%,僅 12 筆樣本先觀察):
#     旗標照算照存(報表 + sig 歸因),樣本累積夠了再決定恢復或移除
#   ※ 月營收 0 觸發非 bug:revenue 快取尚未累積滿 3 個月,滿了會自然開始觸發
#
# 動態門檻調整:
...
# ──────────────────────────────────────────────────────────────
PASS_SCORE = 8             # 滿分 11 的過關門檻 (大盤空頭時自動 +1;RS 失效時自動 -1)
                           # 2026-06-13 由 7 調高:11 分制下 7 分=基本盤+單籌碼,實測一天選出 83 檔太寬。
                           # 8 分=「單籌碼+KD/營收其一」(5+2+1) 或「雙籌碼共振」(5+4=9),寧缺勿濫。
...
def run_screening(
    ...
    print("=" * 60)
    print(">>> 本地計分選股 (滿分 11;法人 3 / 籌碼 4[大戶、散戶各 2;券停用觀察] / 技術 2 / 基本 1 / 大盤 1)")
    ...
    print(f">>> 計分與排序... (門檻 = {effective_pass_score} / 11)")
    ...
    for sid in all_ids:
        ...
        # 滿分 11:法人 3 (投信、外資、雙買) + 籌碼 4 (大戶 2、散戶 2) + 技術 2 (技術面、KD) + 基本 1 + 大盤 1
        # 大戶/散戶各 2 分:訊號歸因實證它們是唯二明顯正 edge(+9.2% / +6.4%),其餘訊號
        # 接近門票性質。門檻 8 下:基本盤(投信+外資+雙買+技術+RS=5)+單籌碼=7 還不夠,
        # 需「單籌碼+KD/營收」或「雙籌碼共振」,籌碼實質為全時段入場條件
        # 券相關停用計分(2026-06-12):歸因 edge -6.34% 但僅 12 筆樣本,先「只記錄不計分」——
        # margin_combined 旗標照算照存(報表欄位 + picks_history sig),累積樣本後再判恢復或移除
    ...
    if results:
        ...
        print("=" * 60)
        print(f"✅ 篩選完成!共 {len(df)} 檔達標 (總分 >= {effective_pass_score} / 11)")
        if chip_gate_on:
