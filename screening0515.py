...
#   7. 技術面三合一(0,停用觀察)  8. KD 低檔金叉(1)   9. 月營收達標(1)
#   10. RS vs 大盤(1) (僅多頭計分;空頭時記 0 — 見動態門檻調整)
#   ※ 大戶/散戶加重為 2 分:訊號歸因唯二明顯正 edge(+9.2%/+6.4%)。
#     門檻 8(2026-06-13 由 7 調高;7 分太寬,實測一天 83 檔):技術面停用後滿分降為 10,
#     門檻維持 8 → 實質收緊;無籌碼訊號(最高 法人3+KD1+營收1+RS1=6)進不來
#   ※ 券相關 2026-06-12 起停用計分(歸因 edge -6.34%,僅 12 筆樣本先觀察)。
#   ※ 技術面三合一 2026-06-19 起停用計分(歸因 edge -4.99%:觸發 -1.44% vs 未觸發 +3.55%,
#     反指標——很可能專挑追高股、拖累勝率)。兩者旗標都照算照存(報表 + sig 歸因),
#     樣本累積夠了再決定恢復/反向用/移除。
...
def run_screening(
    ...
    print("=" * 60)
    print(">>> 本地計分選股 (滿分 10;法人 3 / 籌碼 4[大戶、散戶各 2;券停用觀察] / 技術 1[僅KD,三合一停用] / 基本 1 / 大盤 1)")
    ...
    for sid in all_ids:
        ...
        # 滿分 10:法人 3 (投信、外資、雙買) + 籌碼 4 (大戶 2、散戶 2) + 技術 1 (僅 KD) + 基本 1 + 大盤 1
        # 大戶/散戶各 2 分:訊號歸因實證它們是唯二明顯正 edge(+9.2% / +6.4%),其餘訊號
        # 接近門票性質。門檻 8 下:基本盤(投信+外資+雙買+RS=4)+單籌碼=6 還不夠,
        # 需再湊 KD/營收 或直接「雙籌碼共振(4)+法人」,籌碼實質為全時段入場條件
        # 券相關停用計分(2026-06-12):歸因 edge -6.34%(僅 12 筆樣本),「只記錄不計分」。
        # 技術面三合一停用計分(2026-06-19):歸因 edge -4.99%(觸發 -1.44% vs 未觸發 +3.55%),
        #   反指標、很可能專挑追高股拖累勝率。tech 與 margin_combined 旗標都照算照存
        #   (報表欄位 + picks_history sig),累積樣本後再判恢復/反向用/移除。
