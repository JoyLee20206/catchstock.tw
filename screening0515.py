...
#     門檻 7 分下等於「無籌碼訊號就進不來」(其餘條件加總最高 5 分)
...
# ──────────────────────────────────────────────────────────────
PASS_SCORE = 7             # 滿分 11 的過關門檻 (大盤空頭時自動 +1;RS 失效時自動 -1)
                           # 空頭門檻 8:基本盤 4(RS 關、券不計)+ 大戶 2 = 6 不夠 → 需大戶+散戶雙籌碼(4+4=8)才過
...
def run_screening(
    ...
    for sid in all_ids:
        ...
        # 接近門票性質。加重後門檻 7 分 = 「基本盤(投信+外資+雙買+技術+RS=5)之外,
